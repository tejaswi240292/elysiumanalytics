view: top_10_investors {
  derived_table: {
    sql: select *
       ,Row_number() over (order by Q_Quater)  as tot_q
from
(select   Q_Quater
    ,Investorname
    ,isnull (Q_Num_SHRS,0) Q_Num_SHRS
    ,Q_STATUS
    ,Q_Total_Value
        ,Q_Unit_Price
        ,Q_Share_Diff
    ,Q_Price_Diff
    ,Q_value_Diff
    ,Row_number() over (partition by Q_Quater order by isnull (Q_Num_SHRS,0) desc )  as top_ownership_rnk



      from

      (select distinct q1.investorname  as Investorname,
      ...
      select  q2.investorname  as Investorname,
      ...
      select q3.investorname  as Investorname,
      ...
      select distinct   q4.investorname  as Investorname,
 ...
 group by Q_Quater,Investorname,Q_Num_SHRS,Q_STATUS,Q_Total_Value,Q_Unit_Price,Q_Share_Diff,Q_Price_Diff,Q_value_Diff ) a
 where top_ownership_rnk <=10
 ;;
  }
  ...
  dimension: q_num_shrs {
    type: number
    sql: ${TABLE}.Q_Num_SHRS ;;
  }
  ...
  dimension: q_total_value {
    type: number
    sql: ${TABLE}.Q_Total_Value ;;
  }

  dimension: q_unit_price {
    type: number
    sql: ${TABLE}.Q_Unit_Price ;;
  }

  dimension: q_share_diff {
    type: number
    sql: ${TABLE}.Q_Share_Diff ;;
  }

  dimension: q_price_diff {
    type: number
    sql: ${TABLE}.Q_Price_Diff ;;
  }

  dimension: q_value_diff {
    type: number
    sql: ${TABLE}.Q_value_Diff ;;
  }

  dimension: top_ownership_rnk {
    type: number
    sql: ${TABLE}.top_ownership_rnk ;;
  }
