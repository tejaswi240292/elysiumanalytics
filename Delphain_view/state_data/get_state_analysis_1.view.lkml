...
SELECT   TOP(1)   Symbol,
    ...
    where Trans_Flag= 'Y'
    and MAType = 'EMA'
    AND Mrk_Dte <= (SELECT  Top(1) Mrk_Dte FROM  tbl_Market_condition WHERE Trans_Flag= 'Y' and Mrk_Dte <= {% parameter dte %} and Symbol   = {% parameter symbol %} ORDER BY Mrk_Dte DESC)
...
and c.[Date] = (SELECT  Top(1) Mrk_Dte FROM  tbl_Market_condition WHERE Trans_Flag= 'Y' and Mrk_Dte <= {% parameter dte %} and Symbol   = {% parameter symbol %} ORDER BY Mrk_Dte DESC)
WHERE a.Symbol= {% parameter symbol %}
AND   a.Mrk_Dte <= (SELECT  Top(1) Mrk_Dte FROM  tbl_Market_condition WHERE Trans_Flag= 'Y' and Mrk_Dte <= {% parameter dte %} and Symbol   = {% parameter symbol %} ORDER BY Mrk_Dte DESC)
