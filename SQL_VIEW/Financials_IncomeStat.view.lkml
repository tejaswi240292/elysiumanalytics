    ...
    derived_table: {
      sql: SELECT * FROM
            ...
            ,(CASE WHEN  ABS(shareswa)>= 1000000000 THEN CAST(CAST((shareswa /1000000000) AS DECIMAL(18,2)) AS VARCHAR)+' B'
             ...
             FROM [Key_Fundamentals].[dbo].[K2O_US_Fundamentals_Stag]  WHERE Ticker='AAPL' AND [dimension]='ARY' AND [ReportPeriod]<='2020-08-04' ) a WHERE RowNo=1)
            ...
            ,(CASE WHEN  ABS(shareswa)>= 1000000000 THEN CAST(CAST((shareswa /1000000000) AS DECIMAL(18,2)) AS VARCHAR)+' B'
              ...
              FROM [Key_Fundamentals].[dbo].[K2O_US_Fundamentals_Stag]
              WHERE ID IN(SELECT ID  FROM (SELECT ROW_NUMBER() OVER (PARTITION BY [Ticker],CalendarDate ORDER BY [ReportPeriod] DESC ) AS RowNo,ID
             FROM [Key_Fundamentals].[dbo].[K2O_US_Fundamentals_Stag]  WHERE Ticker='AAPL' AND [dimension]='MRT' AND [ReportPeriod]<='2020-08-04' ) a WHERE RowNo=1)
              ORDER BY [ReportPeriod]  DESC
            ) ABC
         ;;
    }
