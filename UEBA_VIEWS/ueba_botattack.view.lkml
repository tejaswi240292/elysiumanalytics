view: ueba_botattack {
  derived_table: {
    sql: select event_start_date::date as event_date,name as id,type,coalesce(feature_agg:dst_port_count,0) ::bigint DST_COUNT,
      coalesce(feature_agg:src_count,0) ::bigint SRC_COUNT,round(pas) as score,pas_algorithm ::String as pas_algorithm,
      case when pas between 80.0 and 100.0 then 3
            when pas between 60.0 and 79.9  then 2
            when pas between 40.0 and 59.9 then 1
            else 0 end as level
      from UEBA_BOT_ATTACK where {% condition datefilter %} event_start_date {% endcondition %}
       ;;
