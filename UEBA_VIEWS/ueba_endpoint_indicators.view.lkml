view: ueba_endpoint_indicators {
  derived_table: {
    sql: select event_start_date::date as event_date,name as id,type,coalesce(feature_agg:end_point_remote_count,0) ::bigint REMOTE_COUNT,
      coalesce(feature_agg:sent_bytes,0) ::bigint SENTBYTES,round(pas) as score ,pas_algorithm ::string as pas_algorithm ,
      case when pas between 80.0 and 100.0 then 3
            when pas between 60.0 and 79.9  then 2
            when pas between 40.0 and 59.9 then 1
            else 0 end as level
      from UEBA_END_POINT_INDICATORS where {% condition datefilter %} event_start_date {% endcondition %}
       ;;
  ...
  dimension: remote_count {
    type: number
    sql: ${TABLE}."REMOTE_COUNT" ;;
  }
