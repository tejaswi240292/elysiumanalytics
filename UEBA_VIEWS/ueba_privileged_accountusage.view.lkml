view: ueba_privileged_accountusage {
  derived_table: {
    sql: select event_start_date::date as event_date,name as id,type,coalesce(feature_agg:bandwidth,0) ::bigint bandwidth,
      coalesce(feature_agg:privelege_total,0) ::bigint total,round(pas) as score,pas_algorithm ::string as pas_algorithm,
      case when pas between 80.0 and 100.0 then 3
            when pas between 60.0 and 79.9  then 2
            when pas between 40.0 and 59.9 then 1
            else 0 end as level
      from UEBA_PRIVILEGED_ACCOUNT_USAGE where {% condition datefilter %} event_start_date {% endcondition %}
       ;;
  ...
  dimension: bandwidth_and_name {
    ...
    link: {
      ...
      link:
      {
        label: "filter_user_detils_geo"
        url: "/dashboards/82?User={{ id._rendered_value }}"
      }
