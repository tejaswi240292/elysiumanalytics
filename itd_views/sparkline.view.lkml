view: sparkline {
   derived_table: {
     sql:select name,event_date,risk_score  from
    table(FN_ML_OVERALL_AGG('analyst1')) where
    name IN (select name from
    (SELECT name from table(FN_ML_OVERALL_AGG('analyst1'))
    where risk_score>=70 and type={% parameter type1 %}
    and {% condition datefilter %} event_date {% endcondition %}
    )) and  {% condition datefilter %} event_date {% endcondition %}
       ;;
   }
